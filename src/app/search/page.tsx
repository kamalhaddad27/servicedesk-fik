...
import { CheckCircle, XCircle, UserCircle, ArrowLeft, Search as SearchIcon, Ticket as TicketIcon } from "lucide-react"
    ...
    <div className="container max-w-6xl mx-auto py-6 space-y-6">
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 max-w-2xl">
        ...
        <Button type="submit" disabled={isSearching}>
          {isSearching ? <LoadingSpinner className="h-4 w-4 mr-2" /> : <SearchIcon className="h-4 w-4 mr-2" />}
          Cari
        </Button>
      </form>
      ...
      <Tabs defaultValue="tickets" value={activeTab} onValueChange={setActiveTab}>
        ...
        {/* No results state */}
        {hasSearched && !isSearching && activeTab === "tickets" && ticketResults.length === 0 && (
          <div className="text-center py-12">
            <XCircle className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
            <h2 className="text-xl font-semibold mb-2">Tidak ada tiket yang ditemukan</h2>
            <p className="text-muted-foreground">
              Coba gunakan kata kunci yang berbeda atau periksa filter pencarian Anda.
            </p>
          </div>
        )}
        
        {hasSearched && !isSearching && activeTab === "users" && userResults.length === 0 && (
          <div className="text-center py-12">
            <XCircle className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
            <h2 className="text-xl font-semibold mb-2">Tidak ada pengguna yang ditemukan</h2>
            <p className="text-muted-foreground">
              Coba gunakan kata kunci yang berbeda atau periksa filter pencarian Anda.
            </p>
          </div>
