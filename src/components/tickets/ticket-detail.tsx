...
export function TicketDetail({ ticketId }: TicketDetailProps) {
  ...
  const canEdit =
    userRole === "admin" ||
    (userRole === "user" && ticket?.assignedTo === Number(userRole));
      ...
      <Alert variant="destructive">
        ...
        <AlertDescription>
          {error instanceof Error
            ? error.message
            : "Gagal memuat detail tiket. Silakan coba lagi nanti."}
        </AlertDescription>
      </Alert>
    );
  }
            ...
            <Dialog open={quickResolveOpen} onOpenChange={setQuickResolveOpen}>
              ...
              <DialogContent>
                ...
                <DialogFooter>
                  <Button
                    variant="outline"
                    onClick={() => setQuickResolveOpen(false)}
                  >
                    Batal
                  </Button>
                  <Button
                    onClick={handleQuickResolve}
                    disabled={quickResolveTicket.isPending}
                  >
                    {quickResolveTicket.isPending
                      ? "Menyelesaikan..."
                      : "Selesaikan Tiket"}
                  </Button>
                </DialogFooter>
              ...
              {ticket.slaDeadline && (
                <div>
                  ...
                  <div className="mt-1 flex items-center">
                    ...
                    <p className="text-sm">
                      {formatDate(ticket.slaDeadline, "dd MMM yyyy, HH:mm")}
                    </p>
                  </div>
                </div>
              )}
