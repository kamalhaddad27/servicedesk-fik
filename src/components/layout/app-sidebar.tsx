...
import { LayoutDashboard, Ticket, Users, Settings, BarChart3, FileText, User, PlusCircle, Calendar, HelpCircle, LogOut, ChevronDown } from 'lucide-react'
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
...
export function AppSidebar() {
  ...
  const filteredMainNavItems = mainNavItems.filter((item) => userRole && item.roles.includes(userRole))
  const filteredTicketNavItems = ticketNavItems.filter((item) => userRole && item.roles.includes(userRole))
  const filteredAdminNavItems = adminNavItems.filter((item) => userRole && item.roles.includes(userRole))
  const filteredHelpNavItems = helpNavItems.filter((item) => userRole && item.roles.includes(userRole))
          ...
          >
            <SidebarGroupLabel asChild>
              ...
              >
                ...
                {state === "expanded" && (
                  <motion.div
                    animate={openGroups.tickets ? "open" : "closed"}
          ...
          >
            <SidebarGroupLabel asChild>
              ...
              >
                ...
                {state === "expanded" && (
                  <motion.div
                    animate={openGroups.help ? "open" : "closed"}
                    variants={iconVariants}
                  >
                    <ChevronDown className="h-4 w-4" />
                  </motion.div>
          ...
          {state === "expanded" ? (
            ...
            >
              <div className="flex items-center gap-3 rounded-md bg-sidebar-accent/50 p-2">
                ...
                <Button variant="ghost" size="icon" className="ml-auto h-7 w-7 rounded-full" onClick={() => logout()}>
